...
public class RespostaListaExercicio {
	...
	{
		float resultado = peso / (altura * altura);//certo****1
		return resultado;
	}
	...
	{	
		...
		float resultado2 = (baseMaior + baseMenor) * altura  / 2;
		
		return resultado2;//certo*****2
	}
	...
	{
        for (int i = 0; i < array.length; i++) {//certo**********7
            int numero = 0;
	...
	{
		for(int i = 0;i < array.length;i++) {//certo**********8
			if(array[i]%2!=0)
				System.out.print(array[i] + " ");
		}
	}
	...
	{
		for(int i = 0; i < array.length;i++) {//certo*********9
			if (array[i]%2==0) {
				System.out.print(array[i]+" ");	
			}
		}
